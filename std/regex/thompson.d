    ...
    void eval(InputKind kind)(Thread!DataIndex* t, Group!DataIndex[] matches)
    {
        ...
        for(;;)
        {
            ...
            case IR.Bol:
                ...
                {
                    dchar back = s.loopBack(s._index).peek;
                    bool seenNl = !atEnd && s.peek == '\n';
                    if(startOfLine(back, seenNl))
                    {
                        t.pc += IRL!(IR.Eol);
                        break;
                    }
            ...
            case IR.Backref:
                ...
                { //non zero-width backref
                    ...
                    // just keep incrementing till it ends, everyting is tested just once
